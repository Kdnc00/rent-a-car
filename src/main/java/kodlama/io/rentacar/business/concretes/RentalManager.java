...
public class RentalManager implements RentalService {
    ...
    public List<GetAllRentalResponse> gelAll() {
        ...
        List<GetAllRentalResponse> response = rentals.stream().map(rental -> mapper.map(rental, GetAllRentalResponse.class)).toList();
        return response;
    ...
    public CreateRentalResponse add(CreateRentalRequest request) {
        checkIfCarAvaiblable(request.getCarId());
        Rental rental = mapper.map(request, Rental.class);
        rental.setId(0);
        rental.setTotalPrice(getTotalPrice(rental));
        rental.setStartDate(LocalDateTime.now());

        //fiyatı paymenta yolladı
    ...
    private void checkIfCarAvaiblable(int carId) {
